...
class TitleRow(object):
    def __init__(self, parent, name):
        ...
        x = sizer.GetRows()
        self.title = wx.StaticText(parent, -1, name.replace('&', '&&'))
        self.title.SetFont(
            wx.Font(wx.SystemSettings.GetFont(wx.SYS_ANSI_VAR_FONT).GetPointSize(), wx.FONTFAMILY_DEFAULT, wx.NORMAL,
                    wx.FONTWEIGHT_BOLD))
        sizer.Add(self.title, (x, 0), (1, 3), flag=wx.EXPAND | wx.TOP | wx.LEFT, border=10)
        self.line = wx.StaticLine(parent)
        sizer.Add(self.line, (x + 1, 0), (1, 4), flag=wx.EXPAND | wx.LEFT, border=10)
        sizer.SetRows(x + 2)
        # print(self.panel.GetChildren())
        # print(self.panel.GetParent())
...
class SettingRow(object):
    def __init__(self, panel, configName, valueOverride=None, index=None):
        ...
        if self.setting.getExpertSubCategory() is not None:
            ...
            self._expert_button.SetFont(
                wx.Font(wx.SystemSettings.GetFont(wx.SYS_ANSI_VAR_FONT).GetPointSize() * 0.8, wx.FONTFAMILY_DEFAULT,
    ...
    def Hide(self):
        # self.panel.RemoveChild(self.label)
        self.label.Hide()
        self.ctrl.Hide()
