...
class mainWindow(wx.Frame):
    def __init__(self):
        super(mainWindow, self).__init__(None, title='C4M - '+version.getVersion())
        ...
        if wx.Display.GetFromPoint(self.GetPosition()) < 0:
            self.Centre()
        if wx.Display.GetFromPoint((self.GetPositionTuple()[0] + self.GetSizeTuple()[1],
        ...
        # self.scene.SetSize(self.GetSize())

        self.scene.SetFocus()
        self.scene.QueueRefresh()
        # self.scene.SetSize((self.GetSize()[0], self.GetSize()[1] - 40))
    ...
    def onPluginUpdate(self, msg):  # receives commands from the plugin thread
        ...
        if cmd[0] == "OpenPluginProgressWindow":
            ...
            dialogwidth = 300
            dialogheight = 80
            self.dialogframe = wx.Frame(self, -1, cmd[1], pos=(
            (wx.SystemSettings.GetMetric(wx.SYS_SCREEN_X) - dialogwidth) / 2,
            (wx.SystemSettings.GetMetric(wx.SYS_SCREEN_Y) - dialogheight) / 2), size=(dialogwidth, dialogheight),
    ...
    def onOneAtATimeSwitch(self, e):
        ...
        if self.oneAtATime.IsChecked() and profile.getMachineSettingFloat('extruder_head_size_height') < 1:
            wx.MessageBox(_(
                'For "One at a time" printing, you need to have entered the correct head size and gantry height in the machine settings'),
    ...
    def updateMachineMenu(self):
        ...
        for n in xrange(0, profile.getMachineCount()):
            i = self.machineMenu.Append(n + 0x1000, profile.getMachineSetting('machine_name', n).title(),
                                        kind=wx.ITEM_RADIO)
    ...
    def OnLoadProfile(self, e):
        dlg = wx.FileDialog(self, _("Select profile file to load"), os.path.split(profile.getPreference('lastFile'))[0],
    ...
    def OnLoadProfileFromGcode(self, e):
        dlg = wx.FileDialog(self, _("Select gcode file to load profile from"),
                            os.path.split(profile.getPreference('lastFile'))[0],
                            style=wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)
        dlg.SetWildcard("gcode files (*%s)|*%s;*%s" % (
            ...
            for line in f:
                if line.startswith(';CURA_PROFILE_STRING:'):
                    ...
                    if ';{profile_string}' not in profile.getAlterationFile('end.gcode'):
                        profile.setAlterationFile('end.gcode',
    ...
    def OnSaveProfile(self, e):
        dlg = wx.FileDialog(self, _("Select profile file to save"), os.path.split(profile.getPreference('lastFile'))[0],
                            style=wx.FD_SAVE)
        dlg.SetWildcard("ini files (*.ini)|*.ini")
        if dlg.ShowModal() == wx.ID_OK:
            profile_filename = dlg.GetPath()
            if not profile_filename.lower().endswith(
                    '.ini'):  # hack for linux, as for some reason the .ini is not appended.
    ...
    def OnCustomFirmware(self, e):
        if profile.getMachineSetting('machine_type').startswith('ultimaker'):
            wx.MessageBox(_(
                "Warning: Installing a custom firmware does not guarantee that you machine will function correctly, and could damage your machine."),
                          _("Firmware update"), wx.OK | wx.ICON_EXCLAMATION)
        dlg = wx.FileDialog(self, _("Open firmware to upload"), os.path.split(profile.getPreference('lastFile'))[0],
                            style=wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)
    ...
    def OnSize(self, e):
        ...
        # self.scene.SetSize(self.GetSize())
        self.scene.Center()
        print("windows:"+str(self.GetSize()))
        print("scene:"+str(self.scene.GetSize()))
