...
class printWindowPlugin(wx.Frame):
    def __init__(self, parent, printerConnection, filename):
        super(printWindowPlugin, self).__init__(parent, -1,
                                                style=wx.CLOSE_BOX | wx.CLIP_CHILDREN | wx.CAPTION | wx.SYSTEM_MENU | wx.FRAME_FLOAT_ON_PARENT | wx.MINIMIZE_BOX,
    ...
    def _getColoredRect(self, r, g, b):
        for x in xrange(0, self._mapImage.GetWidth()):
            for y in xrange(0, self._mapImage.GetHeight()):
                if self._mapImage.GetRed(x, y) == r and self._mapImage.GetGreen(x, y) == g and self._mapImage.GetBlue(x,
                                                                                                                      y) == b:
                    w = 0
                    while x + w < self._mapImage.GetWidth() and self._mapImage.GetRed(x + w,
                                                                                      y) == r and self._mapImage.GetGreen(
                                    x + w, y) == g and self._mapImage.GetBlue(x + w, y) == b:
                        w += 1
                    h = 0
                    while y + h < self._mapImage.GetHeight() and self._mapImage.GetRed(x,
                                                                                       y + h) == r and self._mapImage.GetGreen(
                            x, y + h) == g and self._mapImage.GetBlue(x, y + h) == b:
                        h += 1
                    return x, y, w, h
    ...
    def _updateButtonStates(self):
        for button in self._buttonList:
            if button.command == self.script_connect:
                ...
                button.Enable(
                    not self._printerConnection.isActiveConnectionOpen() and not self._printerConnection.isActiveConnectionOpening())
...
class printWindowBasic(wx.Frame):
    ...
    def __init__(self, parent, printerConnection):
        super(printWindowBasic, self).__init__(parent, -1,
                                               style=wx.CLOSE_BOX | wx.CLIP_CHILDREN | wx.CAPTION | wx.SYSTEM_MENU | wx.FRAME_TOOL_WINDOW | wx.FRAME_FLOAT_ON_PARENT,
                                               title=_("Printing on %s") % (printerConnection.getName()))
        self._printerConnection = printerConnection
