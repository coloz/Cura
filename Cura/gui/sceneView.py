...
from Cura.gui import configPanel

class SceneView(openglGui.glGuiPanel):
    def __init__(self, parent):
        super(SceneView, self).__init__(parent)
        self.mainWindow = self.GetParent()
        self._yaw = 30
        self._pitch = 60
        self._zoom = 300
        ...
        self.viewSelection = openglGui.glComboButton(self, _("View mode"), [6, 19, 11, 15, 23],
                                                     [_("Normal"), _("Overhang"), _("Transparent"), _("X-Ray"),
                                                      _("Layers")], (-2, 0), self.OnViewChange)
    ...
    def loadFiles(self, filenames):
        ...
        if len(filenames) == 1:
            ...
            if ext == '.g' or ext == '.gcode':
                ...
                self.mainWindow.addToModelMRU(filename)
            ...
            while filenames:
                ...
                if os.path.isdir(filename):
                    ...
                    if ext == '.ini':
                        ...
                        self.mainWindow.addToProfileMRU(filename)
    ...
    def OnPrintButton(self, button):
        if button == 1:
            ...
            #检查有几个串口，如果只有一个，则设为打印串口，如果有多个，则弹出配置窗口，选择串口
            if connectionGroup is not None:
                ...
                # self.showSaveGCode()
                self.OnSettingsButton(2)

    def OnSettingsButton(self, button):
        scw = configPanel.configDialog(lambda: self.mainWindow.scene.sceneUpdated())
        scw.Centre()
        scw.Show()
        if button==2:
            scw.ChangeTab()
    ...
    def showSaveGCode(self, button=1):
        if button == 1:
            ...
            if not self._scene.checkPlatform(newObj):
                if requestedArrangeAll == 0:
                    ...
                    dlg = wx.MessageDialog(self, _(
                        "Cannot fit all the requested duplicates. Do you want to try and reset object positions?"),
                                           _("Reset Positions"), wx.YES_NO)
    ...
    def _updateEngineProgress(self, progressValue):
        ...
        if finished:
            ...
            for e in xrange(0, int(profile.getMachineSetting('extruder_amount'))):
                ...
                if cost is not None:
                    text += '\n%s' % (cost)
            self.sliceButton.setBottomText(text)
        self.QueueRefresh()
            ...
            if e.GetButton() == 3:
                ...
                if len(self._scene.objects()) > 0:
                    ...
                    self.Bind(wx.EVT_MENU, self.OnResetTransformations,
                              menu.Append(-1, _("Reset all objects transformations")))
        ...
        if machine_type not in self._platformMesh:
            ...
            if machine_type == 'mostfun':
                if machine_name == 'mostfun Sail':
                    ...
                    texture_name = 'mostfun_pro_plate.png'
                    texture_scale = 0.4
